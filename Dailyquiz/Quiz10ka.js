...
import { responsiveFontSize } from 'react-native-responsive-dimensions'; // Or your own implementation4
import MrecAdComponent from "../MrecAdComponent";


const Quiz10ka = ({ navigation }) => {
  ...
  const getQuiz = async () => {
    setIsLoading(true);
    const url = 'https://siddiq3.github.io/Api/Quizapi10ka.json';
    try {
  ...
  useEffect(() => {
    if (questions.length && ques < totalQuestions) {
      setOptions(generateOptionsAndShuffle(questions[ques].incorrect_answers.concat(questions[ques].correct_answer)));
      setTimeLeft(15);
    }
  }, [ques, questions, totalQuestions]);
  ...
  const generateOptionsAndShuffle = (optionsArray) => {
  ...
  const handleShowResult = () => {
    ...
    navigation.navigate('Result10ka', {
      score,
      totalQuestions,
      correctQuestions,
      incorrectQuestions,
