...
const class9 = ({ navigation }) => {
    ...
    const saveLastButtonClickTime = async () => {
        try {
            ...
            await AsyncStorage.setItem('lastButtonClickTime3', currentTime.toString());
        } catch (error) {
    ...
    const checkButtonStatus = async () => {
        try {
            const lastButtonClickTime3 = await AsyncStorage.getItem('lastButtonClickTime3');
            if (lastButtonClickTime3) {
                const timeDifference = differenceInMilliseconds(new Date(), new Date(lastButtonClickTime3));
                            ...
                            {isLoading ? (
                                ...
                                <>
                                    <Text style={{ fontSize: responsiveFontSize(2.5), fontWeight: 'bold', color: '#ffffff' }}>
                                        {decodeURIComponent(questions?.t9 || '')}  Quiz
                                    </Text>
                                    <Text style={{ fontSize: responsiveFontSize(2), color: '#ffffff' }}>
                                        Q. {decodeURIComponent(questions?.q9 || '')}?
                                    </Text>
                                </>
                            )}
