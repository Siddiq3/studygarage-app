...
const Class8ka = ({ navigation }) => {
  ...
  const saveLastButtonClickTime = async () => {
    try {
      ...
      await AsyncStorage.setItem('lastButtonClickTime8ka', currentTime.toString());
    } catch (error) {
  ...
  const checkButtonStatus = async () => {
    try {
      const lastButtonClickTime = await AsyncStorage.getItem('lastButtonClickTime8ka');
  ...
  const buttons = [
    { id: 'tb8ka', label: decodeText(questions?.tb8ka), onPress: () => handleNavigate('8thclass tbka') },
    { id: 'imp8ka', label: decodeText(questions?.imp8ka), onPress: () => handleNavigate('8thclass impka') },
    { id: 'fa18ka', label: decodeText(questions?.fa18ka), onPress: () => handleNavigate('8thclass fa1ka') },
    { id: 'fa28ka', label: decodeText(questions?.fa28ka), onPress: () => handleNavigate('8thclass fa2ka') },
    { id: 'sa18ka', label: decodeText(questions?.sa18ka), onPress: () => handleNavigate('8thclass sa1ka') },
    { id: 'fa38ka', label: decodeText(questions?.fa38ka), onPress: () => handleNavigate('8thclass fa3ka') },
    { id: 'fa48ka', label: decodeText(questions?.fa48ka), onPress: () => handleNavigate('8thclass fa4ka') },
    { id: 'sa28ka', label: decodeText(questions?.sa28ka), onPress: () => handleNavigate('8thclass sa2ka') },
  ];
  ...
  return (
    <LegacyHubLayout
      title='Karnataka 8th Class'
      subtitle='Textbook, FA, SA and quick quiz'
      buttons={buttons}
      isLoading={isLoading}
      onQuizPress={setButton}
      quizDisabled={buttonDisabled}
      quizTitle={decodeText(questions?.t8ka)}
      quizQuestion={decodeText(questions?.q8ka)}
